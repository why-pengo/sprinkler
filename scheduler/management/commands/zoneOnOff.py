...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "on_off", type=str, help="Turn on or off the requested zone."
        )
    ...
    def handle(self, *args, **options):
        ...
        if on_off == "on":
            self.log(
                f"scheduler/management/commands/zoneOnOff: turning on, on_off = {on_off}"
            )
            relay_call(zone_map.bcm, 0)
        else:
            self.log(
                f"scheduler/management/commands/zoneOnOff: turning off, on_off = {on_off}"
            )
